...
import { Phone, Lock, Loader, ArrowRight } from 'lucide-react';
...
function Login({ onLogin }) {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      const response = await api.post('/verify-login', { phone, code });
      if (response.data.success) {
      ...
      >
        <div className="backdrop-blur-lg bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-white/70">Login to Dabylicious</p>
          </div>
